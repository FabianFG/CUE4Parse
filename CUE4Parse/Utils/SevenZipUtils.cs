...
public static class Lzma
{
    public static void Compress(Stream input, Stream output, LzmaSpeed speed = LzmaSpeed.Fastest, DictionarySize dictionarySize = DictionarySize.VerySmall, Action<long, long>? onProgress = null)
    {
        ...
        CoderPropID[] propIDs =
        {
            CoderPropID.DictionarySize, CoderPropID.PosStateBits, // (0 <= x <= 4).
            CoderPropID.LitContextBits, // (0 <= x <= 8).
            CoderPropID.LitPosBits, // (0 <= x <= 4).
            CoderPropID.NumFastBytes, CoderPropID.MatchFinder, // "BT2", "BT4".
            CoderPropID.EndMarker
        };

        object[] properties = { (int) dictionarySize, posStateBits, litContextBits, litPosBits, numFastBytes, matchFinder, endMarker };
    ...
    private class DelegateCodeProgress(Action<long, long> handler) : ICodeProgress
    {
        private readonly Action<long, long> _handler = handler;
