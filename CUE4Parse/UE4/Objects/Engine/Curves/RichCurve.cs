...
public class FCompressedRichCurve : IUStruct
{
    ...
    */
    public int NumKeysOrConstant;

    public FRichCurve Curve;

    public FCompressedRichCurve(FAssetArchive Ar)
    {
        CompressionFormat = Ar.Read<ERichCurveCompressionFormat>();
        KeyTimeCompressionFormat = Ar.Read<ERichCurveKeyTimeCompressionFormat>();
        PreInfinityExtrap = Ar.Read<ERichCurveExtrapolation>();
        PostInfinityExtrap = Ar.Read<ERichCurveExtrapolation>();
        NumKeysOrConstant = Ar.Read<int>();
        var CompressedKeys = Ar.ReadArray<byte>();
        unsafe
        {
            fixed (byte* ptr = CompressedKeys)
            {
                var del = UAnimCurveCompressionCodec_CompressedRichCurve.ConverterMap[(int) CompressionFormat][(int) KeyTimeCompressionFormat];
                Curve = del(PreInfinityExtrap, PostInfinityExtrap, NumKeysOrConstant, ptr);
            }
        }
    }
}
