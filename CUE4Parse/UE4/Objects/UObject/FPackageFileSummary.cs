...
public readonly struct FGenerationInfo : ISerializable
{
    /**
...
public class FPackageFileSummary : ISerializable
{
    ...
    private readonly int LegacyFileVersion;
    ...
    internal FPackageFileSummary(FArchive Ar)
    {
        ...
        LegacyFileVersion = CurrentLegacyFileVersion;
    ...
    public void SetCustomVersions(FCustomVersionContainer container)
    {
        CustomVersionContainer = container;
    }

    // NOTE: This is writing back data as an unmodified editor asset for the latest supported version.
    // (UE 5.5 at time of writing)
    // Older engine versions will be supported later
    public void Serialize(FArchiveWriter Ar)
    {
        Ar.Write(Tag);
        Ar.Write(LegacyFileVersion);

        // Doesn't matter if we're unversioned, we're writing editor-compatible data here.
        // We expect versioning info to be set properly.
        const int LegacyUE3Version = 864;
        Ar.Write(LegacyUE3Version);
        Ar.Write(FileVersionUE.FileVersionUE4);
        Ar.Write(FileVersionUE.FileVersionUE5);
        Ar.Write((int) FileVersionLicenseeUE);
        Ar.Serialize(CustomVersionContainer);

        Ar.Write(TotalHeaderSize);
        Ar.WriteFString(FolderName);

        PackageFlags &= ~EPackageFlags.PKG_Cooked; // Should remove cooked flag

        Ar.Write((uint) PackageFlags);
        ...
        if (FileVersionUE >= EUnrealEngineObjectUE5Version.ADD_SOFTOBJECTPATH_LIST)
        {
            Ar.Write(SoftObjectPathsCount);
            Ar.Write(SoftObjectPathsOffset);
        }

        if (FileVersionUE >= EUnrealEngineObjectUE4Version.ADDED_PACKAGE_SUMMARY_LOCALIZATION_ID)
        {
            Ar.WriteFString(LocalizationId);
        }

        if (FileVersionUE >= EUnrealEngineObjectUE4Version.SERIALIZE_TEXT_IN_PACKAGES)
        {
            Ar.Write(GatherableTextDataCount);
            Ar.Write(GatherableTextDataOffset);
        }

        Ar.Write(ExportCount);
        Ar.Write(ExportOffset);
        Ar.Write(ImportCount);
        Ar.Write(ImportOffset);
        Ar.Write(DependsOffset);

        if (FileVersionUE >= EUnrealEngineObjectUE4Version.ADD_STRING_ASSET_REFERENCES_MAP)
        {
            Ar.Write(SoftPackageReferencesCount);
            Ar.Write(SoftPackageReferencesOffset);
        }

        if (FileVersionUE >= EUnrealEngineObjectUE4Version.ADDED_SEARCHABLE_NAMES)
        {
            Ar.Write(SearchableNamesOffset);
        }

        Ar.Write(ThumbnailTableOffset);
        Ar.Serialize(Guid);

        if (FileVersionUE >= EUnrealEngineObjectUE4Version.ADDED_PACKAGE_OWNER)
        {
            Ar.Serialize(PersistentGuid);
        }

        if (FileVersionUE >= EUnrealEngineObjectUE4Version.ADDED_PACKAGE_OWNER && FileVersionUE < EUnrealEngineObjectUE4Version.NON_OUTER_PACKAGE_IMPORT)
        {
            Ar.Serialize(new FGuid()); // empty
        }
        
        Ar.SerializeEnumerable(Generations);

        if (FileVersionUE >= EUnrealEngineObjectUE4Version.ENGINE_VERSION_OBJECT)
        {
            Ar.Serialize(SavedByEngineVersion);
        }
        else
        {
            Ar.Write(0); // EngineChangelist
        }
