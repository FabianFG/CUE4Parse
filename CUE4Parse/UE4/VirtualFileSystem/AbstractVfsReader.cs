...
public abstract partial class AbstractVfsReader(string path, VersionContainer versions) : IVfsReader
{
    protected static readonly ILogger log = Log.ForContext<AbstractVfsReader>();

    public string Path { get; } = path;
    public string Name { get; } = StringUtils.SubstringAfterLast(path.Replace('\\', '/'), '/');
    public IReadOnlyDictionary<string, GameFile> Files { get; protected set; } = new Dictionary<string, GameFile>();
    ...
    public VersionContainer Versions { get; set; } = versions;
    ...
    public static bool IsValidIndex(FArchive reader)
    {
        ...
        if (mountPointLength is > MAX_MOUNTPOINT_TEST_LENGTH or < -MAX_MOUNTPOINT_TEST_LENGTH)
            return false;
