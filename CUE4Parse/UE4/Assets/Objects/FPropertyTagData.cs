    ...
    internal FPropertyTagData(FAssetArchive Ar, string type, string name = "")
    {
        ...
        switch (type)
        {
            case "StructProperty":
                ...
                break;
            case "BoolProperty":
                Bool = Ar.Ver >= EUnrealEngineObjectUE3Version.PROPERTYTAG_BOOL_OPTIMIZATION || (Ar.Game == GAME_APBReloaded && (int)Ar.LicenseeVer >= 32) ? Ar.ReadFlag() : Ar.ReadBoolean();
                break;
            case "ByteProperty":
            case "EnumProperty":
                if (Ar.Ver >= EUnrealEngineObjectUE3Version.BYTEPROP_SERIALIZE_ENUM || (Ar.Game == GAME_APBReloaded && (int)Ar.LicenseeVer >= 32))
