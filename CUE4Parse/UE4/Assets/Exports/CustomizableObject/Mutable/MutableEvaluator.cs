...
public class MutableEvaluator
{
    ...
    public void LoadModelStreamable()
    {
        ...
        foreach (var matchingArchive in Provider.Files.Where((entry, _) => entry.Key.Contains(CustomizableObject.Name + ".", StringComparison.OrdinalIgnoreCase) && (entry.Key.EndsWith(".ubulk") || entry.Key.EndsWith(".uptnl"))))
        {
            if (!bulkArchives.TryGetValue(matchingArchive.Value.Size, out var archive))
                ...
                Log.Debug("Duplicate key found: {0}, Current: {1}, New: {2}", matchingArchive.Value.Size, archive, matchingArchive.Key);
        }
        // number = rom.SourceId % 
    ...
    {
        var reader = GetMutableBulkReader(resourceIndex, DataType.DT_MESH);
        
        return reader == null ? null : new FMesh(reader);
    }

    public FImage LoadImageResource(int resourceIndex)
    {
        var reader = GetMutableBulkReader(resourceIndex, DataType.DT_IMAGE);
        
        return reader == null ? null : new FImage(reader);
    }

    private FArchive? GetMutableBulkReader(int resourceIndex, DataType dataType)
    {
        ...
        {
            Log.Debug("BulkReader missing for FileID: {0}", block.FileId);
    ...
    private string GetMutableBulkDataPath(ulong fileId)
    {
        ...
        return $"{baseDir}{CustomizableObject.Name}_SR_{fileId}.uasset";
    }

    private string GetMutableBulkDataPathTemp(int fileId)
    {
        List<string> fileNames = [".004.ubulk", ".003.ubulk", ".124.uptnl", ".152.ubulk", ".048.ubulk", ".016.ubulk"];
        var coPath = CustomizableObject.GetPathName();
        var baseDir = coPath.SubstringBeforeWithLast('/');
        return $"{baseDir}{CustomizableObject.Name}{fileNames[fileId]}";
    }
}
