...
public class HcaDecoder
{
    ...
    public HcaDecoder(Stream hcaStream, ulong key, ushort subKey)
    {
        ...
        HcaInfo = new HcaInfo()
        {
            Version = _hca.Version,
            HeaderSize = _hca.HeaderSize,
            SamplingRate = _hca.SampleRate,
            ChannelCount = _hca.ChannelCount,
            BlockSize = _hca.FrameSize,
            BlockCount = _hca.FrameCount,
            EncoderDelay = _hca.EncoderDelay,
            EncoderPadding = _hca.EncoderPadding,
            LoopEnabled = _hca.LoopFlag,
            LoopStartBlock = _hca.LoopStartFrame,
            LoopEndBlock = _hca.LoopEndFrame,
            LoopStartDelay = _hca.LoopStartDelay,
            LoopEndPadding = _hca.LoopEndPadding,
            SamplesPerBlock = SamplesPerFrame,
            Comment = _hca.Comment,
            EncryptionEnabled = _hca.CiphType == 56
        };
    ...
    public int TestBlock(byte[] data)
    {
...

        {
            ...
            for (int i = byteStart; i < _hca.FrameSize - 0x02; i++)
            {
                if (buffer[i] != 0)
                    return -1;
            }
        ...
        for (int ch = 0; ch < _hca.ChannelCount; ch++)
        {
            for (int sf = 0; sf < Subframes; sf++)
            {
                for (int s = 0; s < SamplesPerSubframe; s++)
                {
                    float fSample = _hca.Channels[ch].Wave[sf][s];
        ...
        if (blanks == _hca.ChannelCount * frameSamples)
            return 0;

        if (_hca.ChannelCount >= 2)
        {
    ...
    {
        _hca.Random = DefaultRandom;

        for (int i = 0; i < _hca.ChannelCount; i++)
        {
            Channel channel = _hca.Channels[i];
    ...
    private int DecodeBlockUnpack(byte[] data)
    {
        ...
        for (int channel = 0; channel < _hca.ChannelCount; channel++)
        {
            UnpackScaleFactors(_hca.Channels[channel], bitReader, _hca.HfrGroupCount, _hca.Version);
            UnpackIntensity(_hca.Channels[channel], bitReader, _hca.HfrGroupCount, _hca.Version);
            CalculateResolution(_hca.Channels[channel], packedNoiseLevel, _hca.AthCurve, _hca.MinResolution, _hca.MaxResolution);
            CalculateGain(_hca.Channels[channel]);
        }
        ...
        for (int subframe = 0; subframe < Subframes; subframe++)
        {
            for (int channel = 0; channel < _hca.ChannelCount; channel++)
            {
                DequantizeCoefficients(_hca.Channels[channel], bitReader, subframe);
            }
        }
    ...
    private void DecodeBlockTransform()
    {
        for (int subframe = 0; subframe < Subframes; subframe++)
        {
            for (int channel = 0; channel < _hca.ChannelCount; channel++)
            {
                int random = _hca.Random;
                ReconstructNoise(_hca.Channels[channel], _hca.MinResolution, _hca.MsStereo, ref random, subframe);
                _hca.Random = random;
                ReconstructHighFrequency(_hca.Channels[channel], _hca.HfrGroupCount, _hca.BandsPerHfrGroup,
                                         _hca.StereoBandCount, _hca.BaseBandCount, _hca.TotalBandCount, _hca.Version, subframe);
            }

            if (_hca.StereoBandCount > 0)
            {
            ...
            for (int channel = 0; channel < _hca.ChannelCount; channel++)
            {
                ImdctTransform(_hca.Channels[channel], subframe);
            }
        }
    }
