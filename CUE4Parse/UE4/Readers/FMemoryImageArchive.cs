...
public class FMemoryImageArchive(FArchive Ar) : FArchive(Ar.Versions)
{
    protected readonly FArchive InnerArchive = Ar;
    ...
    public IEnumerable<(TKeyType, TValueType)> ReadTMap<TKeyType, TValueType>(Func<TKeyType> keyGetter, Func<TValueType> valueGetter, int keyStructSize, int valueStructSize) where TKeyType : notnull
    {
        var pairs = ReadTSet(() => (keyGetter(), valueGetter()), (keyStructSize + valueStructSize).Align(8));
        return pairs;
    }

    public IEnumerable<TElementType> ReadTSet<TElementType>(Func<TElementType> elementGetter, int elementStructSize)
    {
    ...
    public IEnumerable<TElementType> ReadTSparseArray<TElementType>(Func<TElementType> elementGetter, int elementStructSize)
    {
        var initialPos = Position;
        var dataPtr = new FFrozenMemoryImagePtr(this);
        ...
        var continuePos = Position;
        Position = initialPos + dataPtr.OffsetFromThis;
        var data = new List<TElementType>(dataNum);
