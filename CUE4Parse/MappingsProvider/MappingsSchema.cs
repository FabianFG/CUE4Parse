        ...
        Super = new Lazy<Struct?>(() =>
        {
            ...
            if (superStruct != null)
            {
                if (superStruct is UScriptClass)
                {
                    ...
                    Log.Warning("Missing prop mappings for type {StructName}", superStruct.Name);
                    return null;
                }
...
public class PropertyInfo(int index, string name, PropertyType mappingType, int? arraySize = null) : ICloneable
{
