...
public static class Aes
{
    ...
    private static readonly AesProvider _provider;
    ...
    {
        if (encrypted.Array is null)
            throw new ArgumentException("ArraySegment has no backing array.", nameof(encrypted));
    ...
    public static byte[] DecryptRange(this byte[] encrypted, int beginOffset, int count, FAesKey key)
    {
        ...
        if (alignedEndOffset > encrypted.Length)
            throw new ArgumentException("Partial AES-ECB decryption requires the input buffer to contain the complete surrounding blocks.", nameof(count));
        ...
        return offsetInDecrypted == 0 && count == decrypted.Length ? decrypted : decrypted.AsSpan(offsetInDecrypted, count).ToArray();
    }

    /// <summary>
    /// Decrypts complete AES-ECB blocks in place.
    /// </summary>
    public static void DecryptInPlace(this byte[] encrypted, FAesKey key) => encrypted.DecryptInPlace(0, encrypted.Length, key);
    ...
    public static void DecryptInPlace(this byte[] encrypted, int beginOffset, int count, FAesKey key)
    {
        ...
        using var decryptor = _provider.CreateDecryptor(key.Key, null);
    ...
    public static byte[] CryptCtr(this byte[] input, int beginOffset, int count, FAesKey key, ReadOnlySpan<byte> initializationVector, uint initialBlockIndex = 0, int initialBlockByteOffset = 0)
    {
        ArgumentOutOfRangeException.ThrowIfNegative(beginOffset);
        ArgumentOutOfRangeException.ThrowIfNegative(count);
        if (beginOffset > input.Length - count)
        ...
        var result = beginOffset == 0 && count == input.Length ? input : input.AsSpan(beginOffset, count).ToArray();
    ...
    /// <param name="initialBlockByteOffset">The byte offset within <paramref name="initialBlockIndex"/>.</param>
    public static void CryptCtrInPlace(this Span<byte> data, FAesKey key, ReadOnlySpan<byte> initializationVector, uint initialBlockIndex = 0, int initialBlockByteOffset = 0)
    {
        ...
        if ((uint) initialBlockByteOffset >= ALIGN)
            throw new ArgumentOutOfRangeException(nameof(initialBlockByteOffset), initialBlockByteOffset, $"The byte offset within an AES block must be between 0 and {ALIGN - 1}.");
        if (data.IsEmpty)
            return;
        ...
        try
        {
            using var encryptor = _provider.CreateEncryptor(key.Key, null);
            var processed = 0;
            ...
            if (initialBlockByteOffset != 0)
            {
                ...
                TensorPrimitives.Xor(firstBlock, counter.Slice(initialBlockByteOffset, firstBlockLength), firstBlock);
    ...
    private static void FillCounterBlocks(Span<byte> counters, ReadOnlySpan<byte> initializationVector, uint initialBlockIndex)
    {
        for (var offset = 0; offset < counters.Length; offset += ALIGN)
        {
            ...
            BinaryPrimitives.WriteUInt32BigEndian(counter[CounterPrefixSize..], checked(initialBlockIndex + (uint) (offset / ALIGN)));
        }
    }

    static Aes()
    {
        _provider = AesProvider.Create();
        _provider.Mode = CipherMode.ECB;
        _provider.Padding = PaddingMode.None;
        _provider.BlockSize = BLOCK_SIZE;
    }
}
