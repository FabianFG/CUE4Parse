...
public class FAesKey
{
    ...
    {
        if (key.Length != 32) throw new ArgumentException("Aes Key must be 32 bytes long");
        Key = key;
    ...
    {
        if (!keyString.StartsWith("0x")) keyString = "0x" + keyString;
        if (keyString.Length != 66) throw new ArgumentException("Aes Key must be 32 bytes long");
        KeyString = keyString;
