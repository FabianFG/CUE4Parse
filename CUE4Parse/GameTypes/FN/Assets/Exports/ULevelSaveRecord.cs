...
public class ULevelSaveRecord : UObject
{
    ...
    public override void Deserialize(FAssetArchive Ar, long validPos)
    {
        ...
        else
        {
            ...
            TemplateRecords = GetOrDefault("TemplateRecords", new Dictionary<int, FActorTemplateRecord>());
    ...
    protected internal override void WriteJson(JsonWriter writer, JsonSerializer serializer)
    {
        ...
        writer.WritePropertyName(nameof(SaveVersion));
        writer.WriteValue(SaveVersion);

        writer.WritePropertyName(nameof(bCompressed));
        writer.WriteValue(bCompressed);

        if (!string.IsNullOrEmpty(SemanticVersion))
        {
            writer.WritePropertyName(nameof(SemanticVersion));
            writer.WriteValue(SemanticVersion);
        }

        if (IslandTemplateId is { Length: > 0 })
        {
            writer.WritePropertyName(nameof(IslandTemplateId));
            writer.WriteValue(IslandTemplateId);
        }

        if (NavmeshRequired != 0)
        {
            writer.WritePropertyName(nameof(NavmeshRequired));
        ...
        {
            writer.WritePropertyName(nameof(Center));
            serializer.Serialize(writer, Center);
        }

        if (HalfBoundsExtent != FVector.ZeroVector)
        {
            writer.WritePropertyName(nameof(HalfBoundsExtent));
    ...
    private void DeserializeHeader(FArchive Ar)
    {
        ...
        {
            SemanticVersion = Ar.ReadFString();
        }

        if (SaveVersion >= ELevelSaveRecordVersion.v42)
        {
