...
{
    public FStructFallback[] Functions = Ar.ReadArray(() => new FStructFallback(
        Ar, Ar.ResolveTypeIdentifier("GameplayEventHandlerFunctions")));
}

public class FGameplayEventDescriptor(FAssetArchive Ar) :  FStructFallback(Ar, "GameplayEventDescriptor")
{
    public FStructFallback[] Functions = Ar.ReadArray(() => new FStructFallback(
        Ar, Ar.ResolveTypeIdentifier("GameplayEventDefinition")));
}
