...
public static class Oodle
{
        ...
        byte[] uncompressed, int uncompressedOffset, int uncompressedSize, FArchive? reader = null)
    {
        ...
        if (decodedSize < uncompressedSize)
        {
            ...
            Log.Warning("Oodle decompression just decompressed {Decoded} bytes of the expected {Total} bytes", decodedSize, uncompressedSize);
        }
    }
    ...
    public static async Task<bool> DownloadOodleDll(string? path)
    {
        ...
        try
        {
            ...
            Log.Information("Successfully downloaded oodle dll at \"{Path}\"", dllPath);
            return true;
        }
        catch (Exception e)
        {
