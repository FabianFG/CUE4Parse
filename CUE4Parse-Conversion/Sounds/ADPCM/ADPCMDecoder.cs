...
namespace CUE4Parse_Conversion.Sounds.ADPCM
{
    ...
    {
        public static EAudioFormat GetAudioFormat(byte[] inp)
        {
            using var Ar = new FByteArchive("WhoDoesntLoveCats", inp);
            ...
            if (wvId != EChunkIdentifier.WAVE)
                throw new Exception($"Invalid RIFF identifier (should be {EChunkIdentifier.WAVE} but actually is {wvId})");
            
            var ftId = Ar.Read<EChunkIdentifier>();
            if (ftId != EChunkIdentifier.FMT)
                throw new Exception($"Invalid RIFF identifier (should be {EChunkIdentifier.FMT} but actually is {ftId})");
