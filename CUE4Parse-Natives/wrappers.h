...
DLLEXPORT compressed_tracks* nMakeCompressedTracks(void* buffer, error_result* outErrorResult = nullptr)
{
    return make_compressed_tracks(buffer, outErrorResult);
}

DLLEXPORT error_result nCompressedTracks_IsValid(compressed_tracks* tracks, bool checkHash)
{
    return tracks->is_valid(checkHash);
}

using DecompContextDefault = decompression_context<decompression_settings>;

DLLEXPORT DecompContextDefault* nDecompContextDefault_Create()
{
    return new DecompContextDefault();
}

DLLEXPORT bool nDecompContextDefault_Initialize(DecompContextDefault* context, compressed_tracks& tracks)
{
    return context->initialize(tracks);
}

DLLEXPORT void nDecompContextDefault_Seek(DecompContextDefault* context, int64_t sampleTime, sample_rounding_policy roundingPolicy)
{
    context->seek(sampleTime, roundingPolicy);
}
...
DLLEXPORT UE4OutputTrackWriter* nCreateTrackWriter(FTransform& Atom)
{
    return new UE4OutputTrackWriter(Atom);
}

DLLEXPORT void nDecompContextDefault_DecompressTrack(DecompContextDefault* context, uint32_t trackIndex, UE4OutputTrackWriter& writer)
{
    context->decompress_track(trackIndex, writer);
}

DLLEXPORT void* nAlignedMalloc(size_t size, size_t allignment)
{
